...
//Default Settings
bool modifyBrightness = 0;
int dayBrightness = 80;
int nightBrightness = 20;

int main(void) {

	//Open Config File And Obtain User Settings
	std::ifstream configFile("config.ini");
	std::string line;

	if (configFile.is_open()) {
		while (std::getline(configFile, line)) {
			if (line.find("modifyBrightness=") == 0) {
				modifyBrightness = std::stoi(line.substr(line.find("=") + 1));
			}
			if (line.find("dayBrightness=") == 0) {
				dayBrightness = std::stoi(line.substr(line.find("=") + 1));
			}
			else if (line.find("nightBrightness=") == 0) {
				nightBrightness = std::stoi(line.substr(line.find("=") + 1));
			}
		}
		configFile.close();
	}
	...
	if (GetNightLightStateData(regVal, size)) {
		...
		if (modifyBrightness) { ModifyBrightness(nightLightIsOn); }
...
bool ModifyBrightness(bool nightLightIsOn) {
	...
	if (hDevice != INVALID_HANDLE_VALUE) {
		if (nightLightIsOn) {
			// Asja Computer
			displayBrightness.ucDCBrightness = dayBrightness;
			displayBrightness.ucACBrightness = dayBrightness;
			...
			*/
			...
			// Asja Computer
			displayBrightness.ucDCBrightness = nightBrightness;
			displayBrightness.ucACBrightness = nightBrightness;
